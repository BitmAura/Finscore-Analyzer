...
const ReportPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'accounts' | 'transactions' | 'insights'>('summary');
  ...
  useEffect(() => {
    const checkAuthAndFetchReport = async () => {
      ...
      try {
        ...
        if (jobError) {
          setError(`Failed to fetch report: ${jobError.message}`);
          setIsLoading(false);
          return;
        }
        ...
        if (jobData.status === 'completed') {
          ...
          // Fetch bank accounts related to this job
          ...
          // Fetch sample transactions for this job
  ...
  const handleExportReport = () => {
    ...
    // In a real application, you'd generate a proper PDF report
    let csvContent = `"Report: ${job.report_name}"\n`;
    csvContent += `"Reference ID: ${job.reference_id}"\n`;
    csvContent += `"Generated on: ${new Date().toLocaleString()}"\n\n`;

    if (summary) {
      csvContent += `"Summary:"\n`;
      csvContent += `"Total Transactions","${summary.totalTransactions}"\n`;
      csvContent += `"Total Inflow","₹${summary.totalInflow.toLocaleString()}"\n`;
      csvContent += `"Total Outflow","₹${summary.totalOutflow.toLocaleString()}"\n`;
      csvContent += `"Balance Trend","${summary.balanceTrend}"\n`;
      csvContent += `"Risk Level","${summary.riskLevel}"\n\n`;
    }

    if (bankAccounts.length > 0) {
      csvContent += `"Bank Accounts:"\n`;
      csvContent += `"Bank Name","Account Number","Account Type","Currency"\n`;
      bankAccounts.forEach(account => {
        csvContent += `"${account.bank_name}","${account.account_number}","${account.account_type}","${account.currency}"\n`;
      });
      csvContent += `\n`;
    }
    ...
    if (transactions.length > 0) {
      ...
      transactions.forEach(tx => {
        csvContent += `"${tx.date}","${tx.description}","${tx.debit || ''}","${tx.credit || ''}","${tx.balance}"\n`;
      });
    }

    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8' });
    saveAs(blob, `${job.reference_id}-report.csv`);
  };
