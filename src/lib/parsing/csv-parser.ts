 ...
 * @internal Used internally for type checking during parsing
 */
...
// Define the possible headers we might find in various bank CSVs
...
/**
 * Parses CSV file content and extracts transactions
 * @param fileContent The raw CSV file content as string
 * @returns Promise resolving to array of parsed transactions
 * @public Used by master-parser.ts for CSV file processing
 */
      ...
      complete: (results) => {
        try {
          ...
          if (!dateHeader || !descriptionHeader) {
            return reject(new Error('CSV headers are non-standard. Could not find required columns: Date and Description'));
          }

          if (!debitHeader && !creditHeader && !amountHeader) {
            return reject(new Error('CSV must contain Amount, Debit, or Credit column'));
          }
...
/**
 * Alias for parseCSV - maintained for backward compatibility
 * @public
 */
export const parseCsvToTransactions = parseCSV;
